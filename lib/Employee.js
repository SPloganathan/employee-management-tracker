...
class Employee {
  ...
  async getAllEmployees() {
    ...
    await db
      .promise()
      .query(
        "SELECT employee.id AS id , employee.first_name, employee.last_name, title, name AS department, salary, manager_tb.first_name AS manager FROM employee INNER JOIN role ON employee.role_id = role.id INNER JOIN department ON department.id = role.department_id LEFT JOIN employee manager_tb ON manager_tb.id = employee.manager_id"
      )
      .then(([results]) => {
        data = results;
      })
